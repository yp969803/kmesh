...
type KmeshRecvmsgCompatManagerKey struct {
	NetnsCookie uint64
	_           [8]byte
}

type KmeshRecvmsgCompatOperationUsageData struct {
	StartTime     uint64
	EndTime       uint64
	PidTgid       uint64
...
type KmeshRecvmsgCompatOperationUsageKey struct {
	SocketCookie  uint64
	OperationType uint32
	_             [4]byte
}
...
//
//go:embed kmeshrecvmsgcompat_bpfel.o
